...
function(el, x) {
  ...
  function buildTracerTooltip(feature) {
    ...
    return [
      ...
      'Distance: ' + formatNumber(props.distance, 1) + ' mi'
    ].join('<br>');
  }
  ...
  function drawPartSlow(pts, gradientState, feature, done) {
    ...
    if (!tracer) {
      ...
      tracer.bindTooltip(buildTracerTooltip(feature), {
        direction: 'top',
        offset: [0, -%d],
        opacity: 0.95
      });
      ...
      tracer.setTooltipContent(buildTracerTooltip(feature));
    }
    tracerHeading = null;
...
marker_png_safe, marker_w, marker_h, round(marker_w/2), marker_h, marker_h
))
)
