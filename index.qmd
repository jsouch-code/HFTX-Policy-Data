...
(function(){
  ...
  function setMode(mode){
    if (mode !== "providers") mode = "population";
    if (mode === currentMode && window._txMap && window._txMap.layerManager) return;
    currentMode = mode;
  ...
  // Pick whichever trigger center is closest to viewport center.
  ...
  const timer = setInterval(function(){
    ...
    if ((window._txMap && window._txMap.layerManager) || attempts >= 80) {
      clearInterval(timer);
    }
  }, 150);
  ...
  window.addEventListener('load', updateModeFromScroll);

  // Default map state should match the nearest trigger on load.
  updateModeFromScroll();
})();
</script>
```
