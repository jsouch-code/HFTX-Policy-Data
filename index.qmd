...
<script>
// Switch Leaflet groups (and grouped legends) using explicit closeread triggers.
(function(){
  ...
  // Keep trying briefly until htmlwidgets has initialized the map.
  let attempts = 0;
  const timer = setInterval(function(){
    attempts += 1;
    setMode(currentMode);
    if (window._txMap || attempts >= 60) clearInterval(timer);
  }, 150);

  const mapTriggers = Array.from(document.querySelectorAll('.new-trigger[data-map-mode]'));
  if (mapTriggers.length === 0) return;

  if (window.scrollama) {
    const scroller = window.scrollama();
    scroller
      .setup({ step: '.new-trigger[data-map-mode]', offset: 0.5, progress: false })
      .onStepEnter(function(step){
        const mode = step.element.getAttribute('data-map-mode') || 'population';
        setMode(mode);
      });
    ...
    const observer = new IntersectionObserver(function(entries){
      ...
      if (!bestEl || bestRatio < 0.35) return;
      setMode(bestEl.getAttribute('data-map-mode') || 'population');
    }, { threshold: [0.2, 0.35, 0.6, 0.8] });

    mapTriggers.forEach(function(el){ observer.observe(el); });
  }
