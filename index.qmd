...
function(el, x) {
  var map = this;
  var animationStartDate = new Date(2025, 0, 6);
  ...
  // ---- Controls ----
  var pointsPerPart = 200; // smoothness (try 80–220)
  var partMs = 1400;       // time to draw each part
  var pausePartMs = 120;   // pause between parts
  var pauseRouteMs = 220;  // pause between routes

  // Gradient endpoints:
  // start: #0075A9E6  (RRGGBBAA)
  // end:   #53284F    (RRGGBB)
  ...
  // Layer group to hold all segments (accumulate)
  var group = L.layerGroup().addTo(map);

  // ---- Date card in lower-left ----
  var dateControl = L.control({ position: 'bottomleft' });
  var dateCard;
  dateControl.onAdd = function() {
    dateCard = L.DomUtil.create('div', 'route-date-card');
    dateCard.innerHTML = '<span class="route-date-label">Date</span><span class="route-date-value">January 6, 2025</span>';
    return dateCard;
  };
  dateControl.addTo(map);

  if (!document.getElementById('route-date-card-styles')) {
    var style = document.createElement('style');
    style.id = 'route-date-card-styles';
    style.textContent = [
  ...
  function updateDateCard(orderValue) {
    if (!dateCard) return;
    var routeOrder = Number(orderValue);
    if (!Number.isFinite(routeOrder) || routeOrder < 1) routeOrder = 1;

    var routeDate = new Date(animationStartDate.getTime());
    routeDate.setDate(animationStartDate.getDate() + routeOrder - 1);

    var formattedDate = routeDate.toLocaleDateString('en-US', {
      month: 'long',
      day: 'numeric',
      year: 'numeric'
    });

    var valueEl = dateCard.querySelector('.route-date-value');
    if (valueEl) valueEl.textContent = formattedDate;
  }

  // ---- PNG marker that traces ----
  ...
  var headingSmoothing = 0.18;   // lower = steadier, higher = more reactive
  var headingMinDelta = 2.0;     // ignore tiny jitter changes (degrees)
  ...
  function smoothHeading(targetDeg) {
    if (tracerHeading === null) {
      tracerHeading = targetDeg;
  ...
  // Densify coords -> N points (linear interpolation)
  ...
  // Return parts without stitching (prevents long jump lines)
  ...
  // Draw one part slowly by adding short colored segments
  function drawPartSlow(pts, gradientState, done) {
    if (!pts || pts.length < 2) { done(); return; }

    // Create tracer marker if needed; place at first point
    if (!tracer) {
    ...
    var timer = setInterval(function() {
      ...
      // Progress t for gradient across the entire route
      var t = (gradientState.totalSegs <= 1)
        ? 1
        : (gradientState.drawnSegs / (gradientState.totalSegs - 1));
      var segColor = colorAt(t);

      // Add a short segment from previous point to current point
      ...
      // Move tracer marker to current point
  ...
  function drawRouteSlow(feature, done) {
    ...
    var denseParts = parts.map(function(coords) {
      return densify(coords, pointsPerPart);
    }).filter(function(pts) {
      return pts && pts.length > 1;
    });

    var totalSegs = denseParts.reduce(function(sum, pts) {
    ...
    function nextPart() {
      ...
      drawPartSlow(denseParts[p], gradientState, function(){ p++; nextPart(); });
    }
    nextPart();
  }
  ...
  function nextRoute() {
    ...
    var currentOrder = feats[i] && feats[i].properties ? feats[i].properties.order : (i + 1);
    updateDateCard(currentOrder);
...
",
routes_geojson_txt_safe,
marker_png_safe, marker_w, marker_h, round(marker_w/2), marker_h
))
)
