...
function(el, x) {
  ...
  // ---- Controls ----
  var speedMps = 14;       // tracer speed (meters per second)
  var stepMs = 40;         // animation cadence
  var pointSpacingM = speedMps * (stepMs / 1000);
  ...
  function rotateTracer(angle) {
    if (!tracer) return;
    var icon = tracer.getElement();
    if (!icon) return;
    var t = icon.style.transform || '';
    t = t.replace(/\\srotate\\([^)]*\\)/g, '');
    icon.style.transformOrigin = '50% 100%';
    icon.style.transform = t + ' rotate(' + angle.toFixed(2) + 'deg)';
  }

  function updateTracer(point, angle) {
    tracer.setLatLng([point[1], point[0]]);
    requestAnimationFrame(function(){ rotateTracer(angle); });
  }
  ...
  // Densify coords -> points with near-constant spacing (meters)
  function densify(coords, spacingM) {
    var totalLen = lineLengthM(coords);
    var n = Math.max(2, Math.ceil(totalLen / Math.max(1, spacingM)) + 1);
    var segLens = [], total = 0;
    for (var i=0; i<coords.length-1; i++) {
      var len = map.distance(
        [coords[i][1], coords[i][0]],
        [coords[i+1][1], coords[i+1][0]]
      );
  ...
  function drawPartSlow(coords, done) {
    ...
    var pts = densify(coords, pointSpacingM);
    ...
    if (pts.length > 1) rotateTracer(bearingDeg(pts[0], pts[1]));
    ...
    var timer = setInterval(function() {
      ...
      // Progress t for gradient along this part
      ...
      // Move tracer marker to current point and face direction of travel
      updateTracer(b, bearingDeg(a, b));
  ...
  var observedEl = el.closest('.cr-section') || el;
