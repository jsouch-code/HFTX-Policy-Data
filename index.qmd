...

{
  ...
  function activeNarrativeMode() {
    const mapEl = document.getElementById(mapId);
    if (!mapEl) return "population";

    const section = mapEl.closest(".cr-section") || document;
    const narratives = Array.from(section.querySelectorAll(".narrative-col .trigger .narrative"));
    if (!narratives.length) return "population";
    ...
    if (text.includes("providers per 1,000")) return "providers";
    return "population";
  }
  ...
  async function install() {
    ...
    const update = async () => {
      ticking = false;
      const mode = activeNarrativeMode();
