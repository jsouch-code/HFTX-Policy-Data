...
fpp_cluster_opts <- markerClusterOptions(
  iconCreateFunction = JS(
    "function(cluster) {
      var count = cluster.getChildCount();
      var bg = count < 10 ? 'rgba(102, 187, 106, 0.85)'
               : count < 50 ? 'rgba(67, 160, 71, 0.9)'
               : 'rgba(46, 125, 50, 0.95)';
      return new L.DivIcon({
        html: '<div style=\"background:' + bg + ';\"><span>' + count + '</span></div>',
        className: 'marker-cluster marker-cluster-fpp',
...
htw_cluster_opts <- markerClusterOptions(
  iconCreateFunction = JS(
    "function(cluster) {
      var count = cluster.getChildCount();
      var bg = count < 10 ? 'rgba(255, 183, 77, 0.85)'
               : count < 50 ? 'rgba(255, 152, 0, 0.9)'
               : 'rgba(230, 81, 0, 0.95)';
      return new L.DivIcon({
        html: '<div style=\"background:' + bg + ';\"><span>' + count + '</span></div>',
        className: 'marker-cluster marker-cluster-htw',
        iconSize: new L.Point(40, 40)
      });
    }"
  )
...
m <- leaflet(
  ...
  addCircleMarkers(
    ...
    fillColor = "#66BB6A",
    fillOpacity = 0.9,
    clusterOptions = fpp_cluster_opts,
    group = "FPP Providers"
  ) |>
  addCircleMarkers(
    ...
    fillColor = "#FFB74D",
    fillOpacity = 0.9,
    clusterOptions = htw_cluster_opts,
    group = "HTW Providers"
  ) |>
